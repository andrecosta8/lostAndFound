...
const itemValidation = (req, res, next) => {
    ...
    if (typeof typeOfProduct !== "string"
        ...
        return res.status(401).json({ message: "Please enter a valid type of product(at least 3 letters, only letter and numbers)" })
    }
    if (typeof brand !== "string"
        || brand.match(regex) === null
        || brand.length < 2) {
        return res.status(401).json({ message: "Please enter a valid brand(at least 2 letters, only letters and numbers)" })
    }
    if (typeof color !== "string"
        || color.match(regex) === null
        || color.length < 3) {
        return res.status(401).json({ message: "Please enter a valid color, only letters and numbers" })
    }
